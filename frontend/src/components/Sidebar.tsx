    ...
    <div className="w-72 bg-white border-r border-zinc-200 flex flex-col h-full shrink-0 select-none">
      ...
      <div className="p-4 border-b border-zinc-200 flex items-center justify-between">
        <div className="flex items-center space-x-2.5">
          <div className="w-8 h-8 rounded-lg bg-zinc-100 border border-zinc-200 flex items-center justify-center text-lg shadow-sm">
            🛶
          </div>
          <div>
            <h1 className="font-bold text-sm text-zinc-900 tracking-tight">
              Kayak
            </h1>
          </div>
        </div>
      ...
      {/* Navigation Tabs */}
      <div className="p-2 border-b border-zinc-200 space-y-0.5">
        <button
        ...
        >
          <MessageSquare className={`w-4 h-4 ${currentTab === 'chat' ? 'text-indigo-600' : 'text-zinc-500'}`} />
        ...
        >
          <Bot className={`w-4 h-4 ${currentTab === 'agents' ? 'text-indigo-600' : 'text-zinc-500'}`} />
        ...
        >
          <Sparkles className={`w-4 h-4 ${currentTab === 'skills' ? 'text-indigo-600' : 'text-zinc-500'}`} />
        ...
        >
          <Wrench className={`w-4 h-4 ${currentTab === 'tools' ? 'text-indigo-600' : 'text-zinc-500'}`} />
        ...
        >
          <Settings className={`w-4 h-4 ${currentTab === 'settings' ? 'text-indigo-600' : 'text-zinc-500'}`} />
      ...
      <div className="px-3 pt-3 pb-1 flex items-center justify-between">
        ...
        <button
          ...
          className="p-1.5 rounded-lg text-indigo-600 hover:bg-indigo-50 transition-colors flex items-center gap-1 text-xs font-semibold"
            ...
            return (
              <div
                ...
                className={`group relative flex items-center justify-between px-3 py-2.5 rounded-xl text-xs cursor-pointer transition-all border ${
                  isActive
                    ? 'bg-indigo-50/80 border border-indigo-200 text-zinc-950 font-medium shadow-xs'
              ...
              >
                <div className="flex items-center space-x-2.5 truncate min-w-0 pr-2">
                  <div className="shrink-0">
                    {conversation.isolated_container ? (
                      ...
                      <Cpu className={`w-3.5 h-3.5 ${isActive ? 'text-indigo-600' : 'text-zinc-400'}`} />
                    )}
                  </div>
                  <div className="truncate">
                    <div className="truncate text-xs font-medium text-zinc-900">{conversation.title}</div>
                    <div className="text-[10px] text-zinc-400 flex items-center gap-1.5 mt-0.5 font-mono">
                ...
                <button
                  ...
                  className="opacity-0 group-hover:opacity-100 p-1 text-zinc-400 hover:text-rose-600 hover:bg-zinc-200/60 rounded transition-opacity"
                  title="Delete Conversation"
                >
